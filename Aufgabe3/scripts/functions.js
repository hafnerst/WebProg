...
window.onload = function () {
	...
	var catalogElements = document.getElementsByClassName("catalog");
	for(var i=0; i<catalogElements.length;i++)
	{
		catalogElements[i].addEventListener("click", chooseCat);
	}
...
function chooseCat(event) {
	event.target.style.backgroundColor = "#f8a316";

	if(curCat[0]!=null)
	{
		document.getElementById(curCat[0]).style.backgroundColor = "white";
	}
	curCat[0] = event.target.id;	
}
